...
const ProductCard = ({ product, index, reverse = false }) => {
    ...
    const fallbackFromGallery =
        activeColor && !activeColor.image && galleryImages.length > 0
            ? galleryImages[colorIdx % galleryImages.length]
            : null
    const landscape =
        (activeColor && activeColor.image) ||
        fallbackFromGallery ||
        product.heroImage?.landscape ||
        product.image
